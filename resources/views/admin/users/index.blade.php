    ...
    <table style="width: 100%; border-collapse: collapse;">
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid #e5e7eb;">
                <td style="padding: 12px;">{{ $users->firstItem() + $key }}</td>
                ...
                <td style="padding: 12px;">
                    <button type="button" onclick='editUser(@json($user))' style="background: #f59e0b; color: white; border: none; padding: 6px 10px; border-radius: 6px; cursor: pointer; margin-right: 4px;">
                        <i class="ri-pencil-line"></i>
                    </button>
                    
                    @if(auth()->id() != $user->id)
                    <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Yakin ingin menghapus kasir ini?');">
                        @csrf
                        @method('DELETE')
...
function editUser(user) {
    ...
    document.getElementById('username').value = user.username ? user.username : user.email; 
